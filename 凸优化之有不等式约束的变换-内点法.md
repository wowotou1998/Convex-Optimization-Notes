...
注意一般来说屏障法对应的问题是 $\min_x tf(x)+\phi(x)$，它其实和$\min_x f(x)+\frac{\phi(x)}t$所对应的解是相同的，只是去分母计算起来更方便一些。另外$e_i,d_{i}^{T}$就对应了 $e, D$ 的行元素.
...
要介绍这个方法, 我们要先介绍一下屏障法所引入的一个新的概念——扰动KKT条件 (Perturbed KKT Conditions), 注意，它并不是传统意义上的KKT条件，是经过处理之后得到的一系列新的等式与不等式约束条件。具体来说就是:
...
\begin{cases}
\Delta\lambda_{i}>0,& s \geq \frac{-\lambda_{i}} {\Delta\lambda_{i}};  \because \frac{-\lambda_{i}}{\Delta\lambda_{i}}<0 \therefore s \in [0,1] \\
